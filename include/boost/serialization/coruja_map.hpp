...
inline void save(Archive& ar,
                 const coruja::map<Key, T, Compare, Allocator, Model, Derived>& o,
                 const unsigned int version)
{
    boost::serialization::stl::save_collection<
        Archive, 
        typename coruja::map<Key, T, Compare, Allocator, Model, Derived>::observed_t
        >(ar, o.observed());
}
...
inline void load(Archive& ar,
                 coruja::map<
                     Key, T, Compare, Allocator, Model, Derived>& o,
                 const unsigned int version)
{
    ...
    using Container = typename coruja::map<
        Key, T, Compare, Allocator, Model, Derived>::observed_t;
    ...
    while(count-- > 0)
    {
        ...
        ar.reset_object_address(&(hint->second), &t.reference().second);
        detail::reset_subobject_address(ar, hint->second, t.reference().second);
    }
}
